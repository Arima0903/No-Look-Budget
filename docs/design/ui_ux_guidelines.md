# Orbit Budget: UI/UX & Design Guidelines

本ドキュメントは、「Orbit Budget」アプリにおけるデザインの一貫性（UI/UX）および実装時のルールを定めたガイドラインです。今後の機能追加や画面実装の際には、必ず本指針に沿って実装を行なってください。
...
「Orbit Budget」のコンセプトに従い、操作に必要な手数（タップ数）や文字情報は極限まで減らします。
